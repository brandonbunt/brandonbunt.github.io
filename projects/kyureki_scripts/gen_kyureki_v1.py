...
- Saves images and data.json directly into brandonbunt.github.io/kyureki/
"""
...
# Path to your public kyureki folder
KYUREKI_DIR = os.path.abspath(os.path.join(BASE_DIR, "../../brandonbunt.github.io/kyureki"))
MICROSEASON_IMAGES_DIR = os.path.join(KYUREKI_DIR, "microseason_images")
DATA_FILE = os.path.join(KYUREKI_DIR, "data.json")
IMAGE_NORMAL = os.path.join(MICROSEASON_IMAGES_DIR, "current_microseason.jpg")
IMAGE_DITHERED = os.path.join(MICROSEASON_IMAGES_DIR, "current_microseason_dithered.jpg")
...
def get_today_rokuyo():
    ...
    for td in soup.find_all("td"):
        parts = td.get_text("\n").strip().split("\n")
        if len(parts) == 2 and parts[0].isdigit() and int(parts[0]) == today_day:
